...
// Was SimLog.cachedHTML: one rendered element per (log, cache key). Keyed by the
// log instance so the cache dies with it.
const cachedLogHTML = new WeakMap<SimLog, Record<string, Element>>();

function cacheOutput(log: SimLog, cacheKey: string | number | boolean, fn: () => Element): Element {
	let entries = cachedLogHTML.get(log);
	if (!entries) {
		entries = {};
		cachedLogHTML.set(log, entries);
	}
	if (entries[String(cacheKey)]) return entries[String(cacheKey)];
	entries[String(cacheKey)] = fn();
	return entries[String(cacheKey)]!;
}
...
function renderBaseLog(log: SimLog, includeTimestamp: boolean) {
	return cacheOutput(log, includeTimestamp, () => {
		let html = <>{log.raw}</>;
