...
// `absent` is the port's spelling of `hidden`: it unmounts the box where master left it in the tree
// carrying `hide`. `shown` is still the failure here, and the gem tab below still requires `shown`,
// so a selector that matches nothing fails there rather than passing in both places.
if (atRest.matchingGems === 'shown') problems.push('the matching-gems option is shown on the items tab, expected off');
...
// ---------------------------------------------------------------------------
// The strip's keyboard against the rail's. Both are bound on the same popup and both want arrow
// keys, so this is the one place that holds them apart: Up and Down are the rail's and must never
// move the tab, Left/Right/Home/End are the strip's and must never move the slot.
//
// Up and Down are the recorded difference between the builds, and the whole reason this section
// exists. Bootstrap's `Tab._keydown` treats both axes as prev/next, so on the baseline a focused tab
// swallows Up and Down and the rail never sees them — the rail moves only while focus is elsewhere.
// The React port put the rail handler on the popup and gave the strip no keys at all, so Up and Down
// reached the rail and Left and Right did nothing; Base UI's horizontal composite now takes Left,
// Right, Home and End and leaves Up and Down alone (`COMPOSITE_KEYS` gates entry, but neither is the
// forward or backward key, so `nextIndex` never moves and nothing is prevented or stopped).
//
// So the rail half is asserted on the port only. The strip half is asserted on both, because the
...
say(`  focused     slot=${JSON.stringify(stripStart.slot)} tab=${stripStart.tab}`);
// Where each key lands, not merely that it moved: pressing End twice must stay on the last tab
// rather than read as a key that did nothing.
const LANDS = { ArrowRight: (i, n) => (i + 1) % n, ArrowLeft: (i, n) => (i - 1 + n) % n, End: (_, n) => n - 1, Home: () => 0 };
for (const key of ['ArrowRight', 'ArrowRight', 'End', 'Home', 'ArrowLeft']) {
	...
	say(`  ${key.padEnd(11)} slot=${JSON.stringify(after.slot)} tab=${after.tab}`);
	if (after.slot !== before.slot) problems.push(`${key} on a tab moved the slot ${before.slot} -> ${after.slot}`);
	const lands = LANDS[key](before.tab, before.count);
	if (isReact && after.tab !== lands) problems.push(`${key} from tab ${before.tab} landed on ${after.tab}, expected ${lands}`);
	if (!after.pane) problems.push(`${key} left the modal with no open pane`);
}

// From the first tab, which every slot offers: a slot with fewer tabs than the one before it falls
// back to the first, and that would read as the rail key having moved the strip.
await focusTab();
await page.keyboard.press('Home');
await settle(600);
// Down then up, so the rail has to come back to where it started rather than merely move.
for (const key of ['ArrowDown', 'ArrowUp']) {
	...
	say(`  ${key.padEnd(11)} slot=${JSON.stringify(after.slot)} tab=${after.tab}`);
	if (!isReact) continue;
	if (after.slot === before.slot) problems.push(`${key} on a tab did not move the slot rail (still ${after.slot})`);
	if (after.tab !== before.tab) problems.push(`${key} on a tab moved the strip ${before.tab} -> ${after.tab}`);
}
if (isReact && (await keyState()).slot !== stripStart.slot) problems.push(`the rail did not come back to ${stripStart.slot} after down and up`);
