...
import { hashString, sleep } from '../utils/misc';
...
export class ReforgeGearCache<SpecType extends Spec = Spec> {
	...
	// `env` supplies the page URL that a legacy cached settings link is resolved
	// against (see parseCachedGear); domain code never reads `window` itself.
	constructor(
		playerSpec: PlayerSpec<SpecType>,
	...
	static get<SpecType extends Spec>(playerSpec: PlayerSpec<SpecType>, env: Env): ReforgeGearCache<SpecType> {
		const storeName = ReforgeGearCache.getStoreName(playerSpec);
		let cache = ReforgeGearCache.caches.get(storeName);
		if (!cache) {
			cache = new ReforgeGearCache(playerSpec, env);
	...
	private parseCachedGear(gear: string): EquipmentSpec | null {
		...
		return tryParseUrlLocation(new URL(gear, this.env.location.href))?.settings.player?.equipment || null;
	}
