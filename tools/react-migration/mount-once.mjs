...
for (const spec of specsFromArgv().slice(0, 2)) {
	...
	const ok = r.simUi === 1 && r.rootChildren === 1 && r.stats === 1 && errors.length === 0;
	if (!ok) bad++;
	console.log(
		`${ok ? 'PASS' : 'FAIL'}  ${spec.padEnd(20)} .sim-ui=${r.simUi} #root children=${r.rootChildren} sidebarButtons=${r.sidebarActions} tabs=${r.tabs} stats=${r.stats} errors=${errors.length}`,
	);
