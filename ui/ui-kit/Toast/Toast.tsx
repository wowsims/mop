...
export const Toast = ({ toast, inline = false }: ToastProps) => {
	...
	return (
		<BaseToast.Root
			...
			className={clsx('ui-toast', !inline && 'ui-toast-floating', toast.data?.className)}
			data-testid="sim-toast"
			data-variant={variant}>
			<div className="ui-toast-header">
				<Icon name={VARIANT_ICON[variant]} size="2xl" className="ui-toast-icon" data-testid="sim-toast-icon" />
				<BaseToast.Title className="ui-toast-title" data-testid="sim-toast-title" />
				...
				{canClose && (
					<BaseToast.Close
						render={<Button iconOnly className="ui-toast-close" />}
			...
			<BaseToast.Description render={<div />} className="ui-toast-body" data-testid="sim-toast-body" />
		</BaseToast.Root>
	);
};
