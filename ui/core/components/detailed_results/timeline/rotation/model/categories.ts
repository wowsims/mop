...
// Copied byte-identical from timeline/constants.ts. The table is WotLK-era: 91 of its 242 keys
// never appear as a literal in sim/**/*.go, and on real fixtures it covers 11 of Unholy DK's 28
// cast spell ids and 6 of Demonology's 23 - so the MELEE/SPELL/DEFAULT fallback in actionCategory
// is what actually orders most specs' rows. Flat Record<number, number> keyed by anyId(), so an
// OtherAction id and a spellId of the same numeric value are indistinguishable; safe only because
// OtherAction values are small and spell ids are large.
export const legacyIdToCategoryMap: Record<number, number> = {
...
export function actionCategory(actionId: ActionId, meleeKeys: ReadonlySet<string>, spellKeys: ReadonlySet<string>): number {
	const fixedCategory = legacyIdToCategoryMap[actionId.anyId()];
