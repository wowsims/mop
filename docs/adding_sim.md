...
- Modify `ui/sim/player/specs/$CLASS.ts` to include boilerplate for your `$SPEC` name if it isn't already there.
- Create a directory `ui/specs/$CLASS/$SPEC`. So for a fire mage, `ui/specs/mage/fire`.
...
Each sim is served at `/mop/$CLASS/$SPEC/` and these pages used to be written into the repository.
...
### Custom functionality for one spec

Most of a spec is data — `defineSpec({...})` in `ui/specs/$CLASS/$SPEC/spec.ts`. When a spec needs
behaviour no other spec has, that goes in the `features` slot:

```ts
features: [registerCombustionThresholds],
```

Each entry is a function receiving the `IndividualSimHost` and returning nothing useful. It runs
once, during shell construction, and registers whatever it wants to contribute.

To put something in the sim sidebar, use `host.sidebar`. There are two shapes:

```ts
// A button. Returns a handle whose `update` toggles `disabled` and `loading`.
...
// Anything else. `render` returns React, and the sidebar renders it in place.
host.sidebar.add({
    id: 'my-spec-thing',
    render: () => <MySpecThing host={host} />,
});
```

Entries sort by `order`, which defaults to `0`, and the sort is stable — so entries that leave it
unset appear in the order they registered.

**Write React, not DOM.** The sidebar is rendered by React; an entry that reaches for
`document`, mutates a returned element, or mounts something imperatively is fighting the framework
and will break in ways the parity gates cannot see. The fire mage's combustion-threshold
calculator is the worked example — a button, a progress dialog and a results dialog, all React,
...
Its `.tsx` next to `spec.ts` holds the component; `spec.ts` only names the registration function.
A spec file that needs JSX is named `spec.tsx` instead — the page generator accepts either.

The behaviour slots run in the order `features` → `reforge` → `derivedSettings`, so a feature
cannot read reforge settings during registration; do that inside a handler or an effect, once the
sim is initialised.
