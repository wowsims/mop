...
Frontend = `ui/` (TypeScript). Mid-migration: React 19 is the default JSX dialect, and the
not-yet-ported files opt back out per file to tsx-vanilla JSX → live DOM nodes. For anything about
components, JSX or styling, read the `wowsims-react` skill alongside this one.
...
- 2026-09-04 React migration started; the view layer is now documented by the `wowsims-react`
  ...
  use the automatic runtime, so a new `.tsx` file is a React file; the 94 pre-existing ones opt back
  out with `/** @jsxImportSource @jsx-vanilla */` (shim in `ui/shared/jsx-vanilla/`), and the
  `jsxInject` that used to supply `element`/`fragment` everywhere is gone. Frontend is therefore no
  longer "tsx-vanilla, no framework" — it is React 19 plus a shrinking tsx-vanilla remainder. New
  gate: `npm run test:unit` (vitest + happy-dom). `.oxlintrc.json` gained the react plugin with
  `react-hooks/rules-of-hooks` at error — note it is NOT part of the plugin's default category and
  must stay listed explicitly — which is why two APL builders are now named `makeUse*`.
...
- 2026-09-02 UI restructure PR 9c: the 11 class source dirs moved `ui/<class>/` → `ui/specs/<class>/` via the move tool (`@specs` alias in `tsconfig.json`/`vite.config.mts`/`vite.harness.mts` now resolves to `ui/sims`; `move.mjs`'s `DIR_TO_ALIAS` table key renamed `specs` → `sims`; **superseded 2026-09-07** — `ui/sims` is `ui/specs` again and `@specs` resolves there, so read the directory names in this entry as historical). The generated page location and URL are unchanged: `makefile`'s `PAGE_INDECES` now derives `ui/<class>/<spec>/index.html` from `ui/specs/*/*/spec.ts(x)` (the `ui/%/index.html` rule gained a `mkdir -p $(@D)` since the class/spec dirs no longer pre-exist), and `spec_entry.ts`'s glob/module-key both gained a `sims/` segment. 34/34 golden specs still match.
