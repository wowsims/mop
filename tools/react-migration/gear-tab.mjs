...
// The three summary blocks the pane owns, named rather than discovered: the port unmounts one whose
...
for (const modifier of SUMMARY_MODIFIERS) {
...
const step = async (label, action) => {
	...
	const blocks = Object.fromEntries(summary.map(block => [block.modifier, block]));
	// Every expected block is named whether or not it is there: the port unmounts an empty summary
	// where master hid it, so `absent` is this build's spelling of `1`.
	say(
		`  ${label.padEnd(20)} slots changed=${String(moved).padEnd(3)} hidden=[${SUMMARY_MODIFIERS.map(modifier => `${modifier.replace('summary-table--', '')}:${blocks[modifier] ? Number(blocks[modifier].hidden) : 'absent'}`).join(' ')}]`,
	);
	return { moved, summary: blocks };
};
