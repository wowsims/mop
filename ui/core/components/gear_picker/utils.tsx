...
export const createItemSockets = (player: Player<any>, item: EquippedItem): { elems: HTMLAnchorElement[]; professionSubscription: Disposable | null } => {
	let professionSubscription: Disposable | null = null;
	...
	const elems = item.allSocketColors().map((socketColor, gemIdx) => {
		...
		if (gemIdx === item.numPossibleSockets - 1 && item.couldHaveExtraSocket()) {
			...
			professionSubscription = player.professionChangeEmitter.on(updateProfession);
			updateProfession();
		}
		return gemContainer;
	});
