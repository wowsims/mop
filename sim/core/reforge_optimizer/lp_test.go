...
// TestModelToLPFormatGolden is a golden-output regression test for the LP serializer:
// the fixed model in testdata/lp_parity_model.json must serialize byte-for-byte to the stored
// testdata/lp_parity_golden.lp. If this fails after touching lp.go, the emitted LP text has
// changed, which would alter HiGHS tie-breaking among equal-objective solutions.
func TestModelToLPFormatGolden(t *testing.T) {
	type jsonKV struct {
		K string  `json:"k"`
		V float64 `json:"v"`
	}
	...
	raw, err := os.ReadFile("testdata/lp_parity_model.json")
	if err != nil {
	...
	wantBytes, err := os.ReadFile("testdata/lp_parity_golden.lp")
	if err != nil {
		t.Fatalf("read golden.lp: %v", err)
	}
