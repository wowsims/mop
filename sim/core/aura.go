...
type OnRestore func(aura *Aura, sim *Simulation, stacks int32) // Called when aura state is restored (e.g., via Alter Time) instead of OnGain
...
type Aura struct {
	...
	OnGain          OnGain
	OnExpire        OnExpire
	OnRestore       OnRestore      // Called when aura state is restored (e.g., via Alter Time) instead of OnGain
...
// Internal activate method that optionally triggers OnGain callback.
// Used by RestoreState to activate without triggering OnGain for auras with OnRestore.
