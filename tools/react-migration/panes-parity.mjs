...
for (const spec of specsFromArgv()) {
	const sides = {};
	for (const [side, port] of Object.entries(PORTS)) sides[side] = await openSpec(browser, port, spec, { selector: '.sim-tabs li .nav-link' });

	const ids = await sides.react.page.$$eval('.sim-tabs > li', els => els.map(el => el.className.split(' ')[0]));
	const baseIds = await sides.base.page.$$eval('.sim-tabs > li', els => els.map(el => el.className.split(' ')[0]));
	const problems = [];
	...
	for (const id of ids) {
		...
		for (const side of Object.keys(PORTS)) {
			await sides[side].page.click(`.sim-tabs li.${id} .nav-link`);
			await sides[side].page.waitForTimeout(SETTLE);
			dom[side] = await sides[side].page.evaluate(SERIALIZE, '.sim-main > .tab-pane.active');
		}
		...
		problems.push(`${id}: base ${la.length} elements, react ${lb.length}; first diff line ${first}\n      base : ${la[first]}\n      react: ${lb[first]}`);
	}
