...
- 2026-09-08 **The detailed-results pane is React, and results has its first port.** Nothing in
  ...
  **How a result reaches the pane, end to end**: `addSimResultsAction` subscribes
  ...
  One accepted ordering change: vanilla dropped `dr-no-results` *before* the synchronous emit, while
  the port derives it from `useSimResult()` a microtask later, so `DpsHistogram` measures its row
  while the row is still `display:none`. Measured on both builds rather than assumed — Chart.js's
  `responsive` resize corrects it and the canvas is identical at +150 ms.
