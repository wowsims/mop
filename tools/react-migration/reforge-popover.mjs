...
//
// A tooltip whose own anchor holds focus also comes out, because it is being asked for rather than
// left over: Base UI returns focus to the trigger as the popover closes and react-tooltip opens on
// focus, so pressing Escape with focus inside the popup leaves the cog focused and showing its
// tooltip. The baseline never does that — a tippy popover restores no focus at all — and returning
// focus is the better behaviour, so this is an intended difference, decided 2026-09-11.
const openTooltips = selector =>
	[...document.querySelectorAll('.tippy-box, .sim-tooltip')]
		...
		.filter(el => !el.id || document.activeElement?.getAttribute('data-tooltip-id') !== el.id)
		.filter(el => {
