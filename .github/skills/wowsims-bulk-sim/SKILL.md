...
- Generic cache storage: ui/sim/reforge_cache.ts (sync hashString from ui/sim/utils.ts; values are plain `equipmentSpec:`-prefixed proto JSON; age-index-ranged prune).
...
- Generated constants (never hand-edit; `go run ./tools/database/gen_db -gen=go-to-ts`): ui/core/components/individual_sim_ui/bulk/constants_auto_gen.ts (slot maps) and ui/sim/wasm/bulk_sim/constants_auto_gen.ts (tuning constants + stage ladder), emitted by tools/database/gen_bulksim_constants.ts.go from the exported consts in sim/core/bulk.
...
- The single significance threshold lives in ui/sim/utils.ts (Z_95, zTest) and sim/core/bulk/statistics.go (bulkSimZ95); keep them equal.
