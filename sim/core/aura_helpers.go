...
func (character *Character) NewTemporaryStatBuffWithStacks(config TemporaryStatBuffWithStacksConfig) (*StatBuffAura, *Aura) {
	...
	if config.TimePerStack > 0 {
		...
		// Helper to create the periodic action for stack changes
		...
		aura := character.RegisterAura(Aura{
			...
			OnRestore: func(aura *Aura, sim *Simulation, stacks int32) {
				...
				// Use the stacks parameter to calculate remaining ticks.
				...
				if config.DecrementStacks {
					// For decrementing auras, remaining ticks = stacks being restored
					remainingTicks = int(stacks)
				} else {
					// For incrementing auras, remaining ticks = max - stacks being restored
