...
export const useLegacyMount = (create: (parent: HTMLElement) => Component | Array<Component>, deps: ReadonlyArray<unknown> = []) => {
	...
	return useCallback((parent: HTMLElement | null) => {
		...
		const components = Array.isArray(made) ? made : [made];
		return () => {
