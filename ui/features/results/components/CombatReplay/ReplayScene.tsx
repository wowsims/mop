...
export const ReplayScene = ({ model, clock }: ReplaySceneProps) => (
	<ReplayFramesContext value={clock.frames}>
		<div className="cr-scene relative isolate flex min-h-0 flex-1 flex-col overflow-hidden bg-cr-scene">
			<div className="cr-scene-vignette absolute inset-0 z-0 bg-cr-vignette pointer-events-none" aria-hidden="true" />
			<div className="cr-arena relative z-1 min-h-[180px] flex-1 overflow-hidden bg-transparent">
				<div className="cr-ticker-outer absolute inset-x-0 top-[10px] z-10 flex justify-center">
					<ReplayTicker actions={model.actions} />
				</div>
				<div className="cr-enemy-zone absolute inset-x-[20px] bottom-0 top-[calc(var(--spacing-cr-icon)+20px)] z-1 overflow-hidden">
					<ReplayEnemyFormation enemies={model.enemies} hiddenCount={model.hiddenEnemyCount} />
				</div>
			</div>
			<div className="cr-cdm relative z-1 w-1/2 shrink-0 self-center bg-black-92 px-[16px] py-[8px] border-t border-white-8 flex flex-col gap-[6px]">
				<div className="cr-cdm-header flex min-h-[46px] flex-nowrap items-center gap-[8px]">
					<div className="cr-cdm-player-label shrink-0 text-[0.8rem] font-bold tracking-[0.04em] text-white-70">{model.playerName}</div>
					<ReplayAuraIcons
						className="cr-buff-icons flex w-full min-w-0 flex-1 flex-row flex-wrap content-start justify-start gap-[3px] min-h-[45px] max-h-[132px] overflow-hidden"
