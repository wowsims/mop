...
describe('ReplayIcon', () => {
	it('paints the action’s icon and links to it, hardened against the cross-origin opener', () => {
		const icon = mount(<ReplayIcon actionId={actionId('Fireball')} className="icon-a" tooltip="spell" />);
	...
	it('takes a class list and keeps the caller’s own style alongside the icon', () => {
		const icon = mount(<ReplayIcon actionId={actionId('Fireball')} className={['icon-a', 'icon-a-active']} tooltip="spell" style={{ opacity: 0.5 }} />);
	...
	it('links nowhere and shows no tooltip for a cast with no action behind it', () => {
		...
		const icon = mount(<ReplayIcon actionId={null} className="icon-c" tooltip="spell" />);
	...
	it('resolves an aura’s tooltip against the buff and a cast’s against the spell', async () => {
		tooltipData.mockClear();
		const cast = mount(<ReplayIcon actionId={actionId('Fireball')} className="icon-a" tooltip="spell" />);
		...
		mount(<ReplayIcon actionId={actionId('Enrage')} className="icon-d" tooltip="buffAura" />);
	...
	it('renders into the anchor its caller passed, and its own when there is none', async () => {
		...
		const icon = mount(<ReplayIcon actionId={actionId('Fireball')} className="icon-d" tooltip="buffAura" anchorRef={anchorRef} />);
