...
**/simplify pass (2026-09-02)**: four review angles (reuse / simplification / efficiency / altitude) over the full branch diff, findings deduped and applied: `InputConfig.storeSubscribe` is curried (`obj => StoreSubscribe`) and optional — 141 eta-expansion wrappers and the 16 `aplChildSubscribe` no-op fakes deleted; `subscribeAll` folds selector sources into ONE tuple selector (a real regression vs master's EventID dedup: the stats sidebar rebuilt twice per talent change); raid tuple memoized per state object; `patchSlice`/`patchKeyed`/`seedKeyed`/`deleteKeyed` in sim_store.ts replace ten hand-rolled `setState` shapes and merge value write + counter bump into one store write (reforge/stat-weight changes were 2–3 writes each); slice fields typed (28 casts gone); `UISettings` folded into `Sim`; `Component.disposeChild`; `SavedDataManager` compares JSON only (`equals` config dropped everywhere); results-manager `changeEmitter`/`partOfSwap` deleted (zero subscribers); bulk slice keeps counters only; `reconcile()` memoized; dead exports/markers removed (`isBatching`, `onAnyEmitter`, `subscribeRaidComp`, `within-raid-sim-hide`, `undershootCaps` counter, reforge forwarders + re-export shim, `prevResultData`/time-ruler fields, duplicate `beforeunload`); touched files formatted with oxfmt. Deliberately NOT applied: `Party.setPlayer` discard detection (user decision), `Party.players` mirror (documented), encounter targets version counter, stat-weight self-persistence move, timeline `resultKey` without the chart (plot options differ per chart), plan-doc deletion, scoping oxlint `args: "none"` (would flag hundreds of unused `eventID`/`player` callback params).
