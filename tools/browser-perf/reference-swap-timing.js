...
(async (page) => {
  ...
  await page.waitForFunction(() => document.querySelectorAll('.rotation-pane .rotation-row').length > 0);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    swaps.push(await page.evaluate(() => { window.__po.disconnect(); window.__mo.disconnect(); return { sync: window.__m.sync, settle: Math.round(window.__m.last - window.__m.t0), muts: window.__m.muts, longTasks: window.__m.long, dps: document.querySelector('.results-sim-dps .topline-result-avg').textContent, rows: document.querySelectorAll('.rotation-pane .rotation-row').length, tippy: [...document.querySelectorAll('.rotation-pane *')].filter(e => e._tippy).length }; }));
  }
  // hover a timeline cast to prove tooltips are alive after swaps
  const tooltipAlive = await page.evaluate(() => { const el = [...document.querySelectorAll('.rotation-pane *')].find(e => e._tippy); if (!el) return null; el._tippy.show(); const shown = !!document.querySelector('[data-tippy-root]'); el._tippy.hide(); return shown; });
