# ui/domain/state — the framework-agnostic state layer

Everything in this directory is UI-free (a lint rule bans imports from
`ui/core/components`, and `@ui-kit`/`@features`/`@app`) **and browser-free**: `no-restricted-globals` bans
...
## Not here (facades, one level up in `ui/domain/`)
