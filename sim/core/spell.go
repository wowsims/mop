...
func (spell *Spell) CanCast(sim *Simulation, target *Unit) bool {
	...
	if (spell.Unit.Hardcast.Expires > sim.CurrentTime || spell.Unit.IsChanneling()) && !spell.Flags.Matches(SpellFlagCastWhileChanneling) && (spell.Unit.Rotation.interruptChannelIf == nil || !spell.Unit.Rotation.interruptChannelIf.GetBool(sim)) {
		//if sim.Log != nil {
